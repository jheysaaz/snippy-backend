...
- 🔐 Secure authentication (Argon2id password hashing)- ✨ Create, read, update, and delete code snippets

- 🆔 UUID-based user IDs- 👤 User management with secure password authentication (Argon2id)

- 📝 Full CRUD operations for snippets and users- 🔐 Login system with username or email and password

- 🔍 Full-text search and filtering- 🆔 UUID-based user IDs (automatically generated by PostgreSQL)

- ⚡ Optimized PostgreSQL with connection pooling- � Optimized PostgreSQL database with strategic indexes

- 🌐 CORS-enabled for browser extensions- 🔍 Full-text search capabilities

- 🏷️ Array-based tag system with GIN indexing

## Quick Start- 🎯 Query filtering by language, tags, and search terms

- ⚡ Connection pooling for high performance

````bash- 🌐 CORS enabled for Chrome extension integration

# Start PostgreSQL- 🔄 Auto-updating timestamps via database triggers
...
- `POST /api/v1/auth/login` - Login with username or email and password
...
- Password hashes excluded from API responses    "login": "john@example.com",  // Can use username or email
...
├── main.go           # Server setup and routing    "password": "newsecurepassword456"

├── database.go       # Schema initialization  }'

├── models.go         # Data structures```

├── handlers.go       # Snippet endpoints

├── user_handlers.go  # User/auth endpoints### Get User by Username

├── auth.go           # Password hashing

└── *_test.go         # Test files```bash
...
```
.
├── main.go             # Application entry point, PostgreSQL connection, and router setup
├── database.go         # Database schema initialization and helper functions
├── models.go           # Data models and request/response structures
├── handlers.go         # HTTP handlers for snippet endpoints
├── user_handlers.go    # HTTP handlers for user and authentication endpoints
├── auth.go             # Password hashing (Argon2id) and authentication utilities
...
- **[📖 API Reference](./docs/API.md)** - Detailed API endpoints with examples
- **[🔐 Authentication Guide](./docs/AUTHENTICATION.md)** - JWT authentication & authorization
- **[�️ Security Documentation](./docs/SECURITY.md)** - Security audit & best practices
- **[�💻 Development Guide](./docs/DEVELOPMENT.md)** - Setup, testing, and workflows
- **[🏗️ Architecture](./docs/ARCHITECTURE.md)** - System design and components
- **[🗄️ Database](./docs/DATABASE.md)** - Schema, indexes, and optimizations
